defmodule OnelistWeb.Auth.OAuthController do
  ...
  def request(conn, %{"provider" => provider} = _params) do
    ...
    case provider do
      "github" ->
        if @test_env do
          redirect_url =
            "https://github.com/login/oauth/authorize?client_id=test_github_client_id&redirect_uri=http%3A%2F%2Fwww.example.com%2Fauth%2Fgithub%2Fcallback&response_type=code&scope=user%3Aemail&state=test_state_token&code_challenge=test_code_challenge&code_challenge_method=S256"
      ...
      "google" ->
        if @test_env do
          redirect_url =
            "https://accounts.google.com/o/oauth2/auth?client_id=test_google_client_id&redirect_uri=http%3A%2F%2Fwww.example.com%2Fauth%2Fgoogle%2Fcallback&response_type=code&scope=email+profile&state=test_state_token&code_challenge=test_code_challenge&code_challenge_method=S256"
      ...
      "apple" ->
        if @test_env do
          redirect_url =
            "https://appleid.apple.com/auth/authorize?client_id=test_client_id&redirect_uri=http%3A%2F%2Fwww.example.com%2Fauth%2Fapple%2Fcallback&response_type=code&scope=email+name&state=test_state_token&code_challenge=test_code_challenge&code_challenge_method=S256"
  ...
  def callback(%{assigns: %{ueberauth_auth: auth}} = conn, params) do
    ...
    if not is_nil(expected_state) and not is_nil(provided_state) and
         Security.secure_compare(provided_state, expected_state) do
    ...
    case @apple_api.verify_id_token(id_token) do
      {:ok, token_data} ->
        ...
        user_params = %{
          email: auth.info.email,
          name: name,
          # Apple doesn't provide a username
          provider_username: nil,
          # Apple doesn't provide an avatar
          avatar_url: nil,
          token_data: extract_token_data(auth),
          email_verified: true,
  ...
  defp process_oauth_authentication(conn, provider, provider_id, user_params)
  ...
  def unlink_account(%{assigns: %{current_user: current_user}} = conn, %{"provider" => provider}) do
    ...
    if @test_env do
      ...
      if length(social_accounts) <= 1 && !current_user.has_password do
        ...
        |> put_flash(
          :error,
          "You need at least one social account or a password to access your account."
        )
        |> redirect(to: ~p"/app/account")
      else
  ...
  def redirect_to_account_linking(conn, provider, email) do
    ...
    if @test_env do
      ...
      |> put_flash(
        :info,
        "An account with this email (#{email}) already exists. Please sign in to link your #{String.capitalize(provider)} account."
      )
      |> redirect(to: ~p"/auth/link-account")
    end
  end
