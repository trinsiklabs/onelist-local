defmodule OnelistWeb.Api.V1.RiverController do
  ...
  def context(conn, _params) do
    ...
    |> json(%{
      ...
      recent_entries: Enum.map(context.recent_entries, fn e ->
        %{
          id: e.public_id,
          title: e.title,
          type: e.entry_type,
  ...
  def capture(conn, %{"content" => content} = params) do
    ...
    {:ok, entry} = Onelist.Entries.create_entry(user, %{
    ...
    {:ok, _} = Onelist.Entries.add_representation(entry, %{
      type: "plaintext",
      content: content
    })
    
    response = if clarify do
      # Ask River to help clarify
      {:ok, clarification} = Agent.chat(user, 
