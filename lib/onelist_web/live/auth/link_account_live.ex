defmodule OnelistWeb.Auth.LinkAccountLive do
  ...
  def mount(_params, session, socket) do
    ...
    if pending_link do
      ...
      pending_link =
        for {key, val} <- pending_link, into: %{} do
  ...
  def render(assigns) do
    ...
    <div class="link-account-container">
      ...
      <p class="mb-4">
        You're trying to sign in with <strong><%= String.capitalize(@provider) %></strong>
        using an email
  ...
  def handle_event("link-accounts", _params, socket) do
    ...
    {:noreply,
     ...
     |> put_flash(
       :info,
       "Please sign in to link your account with #{String.capitalize(socket.assigns.provider)}."
     )
     |> redirect(to: ~p"/login")}
  end
  ...
  def handle_event("create-new-account", _params, socket) do
    ...
    {:ok, user} =
      Accounts.create_user(%{
        ...
        # Consider email verified from provider
        email_verified: true
      })

    # Link the social account
    {:ok, _social_account} =
      Accounts.create_social_account(user, %{
        provider: provider,
