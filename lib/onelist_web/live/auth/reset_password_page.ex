  ...
  def handle_params(%{"token" => token}, _uri, socket) do
    ...
    case Accounts.rate_limited?("reset_page", ip_address) do
      {:ok, false} ->
        ...
        case Accounts.get_user_by_reset_token(token) do
          ...
          {:error, _} ->
            ...
            socket = socket
              |> put_flash(:error, "The password reset link is invalid. Please request a new one.")
              |> push_navigate(to: ~p"/forgot-password")
            {:noreply, socket}
        end
      ...
      {:error, :rate_limited, timeout} ->
        ...
        socket = socket
          |> put_flash(:error, "Too many attempts. Please try again in #{div(timeout, 60)} minutes.")
  ...
  defp process_reset(token, password, user, pid) do
    case Accounts.reset_password(user, token, %{password: password}) do
      ...
      {:error, :invalid_token} ->
        ...
        send(pid, {:reset_complete, :error, "Reset link is no longer valid. Please request a new one."})
      ...
      {:error, _} ->
        ...
        send(pid, {:reset_complete, :error, "An error occurred. Please try again or request a new reset link."})
    end
  end
      ...
      <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <% else %>
            ...
            <%= if @token do %>
              <.form for={@form} id="reset_password_form" phx-change="validate" phx-submit="submit" data-test-id="reset-password-form">
                <div class="space-y-6">
                  <div>
