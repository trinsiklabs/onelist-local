defmodule Onelist.River.Agent do
  ...
  defp gtd_context(gtd_state) do
    nudges = case gtd_state[:nudges] do
      nil -> ""
      [] -> ""
      list -> "\nNudges:\n" <> Enum.map_join(list, "\n", &("- #{&1}"))
    end
  ...
  defp search_relevant_memories(%User{id: user_id}, query, opts) do
    ...
    case Searcher.search(user_id, query, limit: limit) do
      {:ok, %{results: results}} when is_list(results) -> 
        ...
        entries_map = if entry_ids != [] do
          Onelist.Repo.all(
            from e in Entries.Entry,
              where: e.id in ^entry_ids,
              select: {e.id, %{public_id: e.public_id, title: e.title, inserted_at: e.inserted_at}}
          ) |> Map.new()
        else
          %{}
        end
    ...
    |> Enum.each(fn line ->
      ...
      unless json == "[DONE]" do
        case Jason.decode(json) do
          {:ok, %{"choices" => [%{"delta" => %{"content" => content}} | _]}} when content != nil ->
