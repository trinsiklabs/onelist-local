defmodule Onelist.GTD do
  ...
  def next_actions(%User{id: user_id}, opts \\ []) do
    ...
    query = if context, do: where(query, [e], fragment("metadata->>'context' = ?", ^context)), else: query
    query = if energy, do: where(query, [e], fragment("metadata->>'energy' = ?", ^energy)), else: query
  ...
  def projects(%User{id: user_id}, opts \\ []) do
    ...
    query = if include_complete do
      query
    else
      where(query, [e], fragment("metadata->>'status' != ? OR metadata->>'status' IS NULL", "complete"))
    end
  ...
  def due_soon(%User{id: user_id}, days \\ 7) do
    ...
    Repo.all(
      from e in Entry,
        where: e.user_id == ^user_id,
        where: fragment("metadata->>'status' IN ('next', 'waiting')"),
        where: fragment("metadata->>'due_date' >= ? AND metadata->>'due_date' <= ?", ^today_str, ^future),
        order_by: [asc: fragment("metadata->>'due_date'")]
    )
  end
  ...
  def project_health(%Entry{id: project_id, user_id: user_id} = project) do
    ...
    items = Repo.all(
      from e in Entry,
        join: l in Onelist.Entries.EntryLink, on: l.source_entry_id == e.id,
  ...
  defp generate_nudges(inbox, next, overdue, stuck, days_since_review) do
    ...
    nudges = if length(inbox) > 10, do: ["Inbox has #{length(inbox)} items - time to clarify?" | nudges], else: nudges
    nudges = if length(inbox) > 0 and length(next) == 0, do: ["No next actions defined - what's the next step?" | nudges], else: nudges
    nudges = if length(overdue) > 0, do: ["#{length(overdue)} overdue items need attention" | nudges], else: nudges
    nudges = if length(stuck) > 0, do: ["#{length(stuck)} projects have no next action" | nudges], else: nudges
    
    # Weekly review nudge
    nudges = case days_since_review do
