defmodule Onelist.Auth.AppleAuth do
  ...
  """
  @callback verify_token_with_key(token :: binary, key :: map, client_id :: binary) :: {:ok, map} | {:error, binary}
  
  @doc """
  Verifies token claims, including issuer, audience, and expiration.
  """
  @callback verify_token_claims(claims :: map, client_id :: binary) :: {:ok, map} | {:error, binary}
...
defmodule Onelist.Auth.Apple do
  ...
  """
  @impl Onelist.Auth.AppleBehaviour
  def extract_user_name(%{raw_info: %{user: %{"name" => %{"firstName" => first, "lastName" => last}}}}),
    do: "#{first} #{last}"
